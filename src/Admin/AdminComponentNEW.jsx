...
function AdminComponent() {
    ...
    const [patternFileURL, setPatternFileURL] = useState('');
    ...
    // Add a global variable to store the uploaded image
let uploadedImage;
...
const handleMarkerUpload = (event) => {
    ...
    reader.onload = (e) => {
        ...
        THREEx.ArPatternFile.encodeImageURL(imageUrl, (patternFileString) => {
            // Generate pattern file
            

            // Display the uploaded image in the floating panel
            ...
            floatingPanel.appendChild(imageContainer);

            // Set pattern file URL for download button
            const url = window.URL.createObjectURL(new Blob([patternFileString], {type: 'text/plain'}));
            setPatternFileURL(url);
                            ...
                            <td>
                                {editingMemberId === member.id ? (
                                    ...
                                    <>
                                        ...
                                        <button onClick={() => setShowMarkerUpload(true)}>Upload Marker...</button>
                                    </>
                                )}
                            </td>
                        </tr>
                    ...
                    {showNewMemberForm && (
                        <tr>
                            ...
                            <td>
                                ...
                                <button onClick={() => setShowFloatingPanel(true)}>Upload Marker...</button>
                            </td>
                        </tr>
                    )}
                </tbody>
            ...
            {showFloatingPanel && (
                <div className="floating-panel-overlay">
                    <div className="floating-panel">
                        ...
                        <input type="file" onChange={handleMarkerUpload} />
                        {showDownloadButton && (
                            <button onClick={handleDownloadPattern}>Download Pattern</button>
                        )}
                        <button onClick={() => setShowMarkerUpload(false)}>Close</button>
                    </div>
                </div>
            )}
        </div>
